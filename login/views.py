...
def savings(request: HttpRequest) -> HttpResponse:
    ...
    for g in goals_list:
        if g.last_allocated_month != current_month_str:
            ...
            g.current_month_auto_allocation = Decimal(str(round(min(base_saved, float(g.target_amount)), 2)))
    ...
    for g in goals_list:
        effective_saved = _goal_effective_saved(g)
...
def api_goal_allocations(request: HttpRequest) -> JsonResponse:
    try:
        ...
        allocation_data = []
        for goal in goals:
            effective_saved = _goal_effective_saved(goal)
...
def _goal_effective_saved(goal: "SavingsGoal") -> float:
    target = float(goal.target_amount or 0)
    saved = float(goal.saved_amount or 0)
    current_auto = float(getattr(goal, "current_month_auto_allocation", 0) or 0)
    return min(max(saved + current_auto, 0), target)


def _goal_remaining_amount(goal: "SavingsGoal") -> float:
    return max(float(goal.target_amount or 0) - _goal_effective_saved(goal), 0)


def _goal_is_fundable(goal: "SavingsGoal") -> bool:
...
def _sync_goal_completion(goal: "SavingsGoal", commit: bool = True) -> bool:
    ...
    effective_saved = goal.saved_amount + goal.current_month_auto_allocation
    fields = []

    if effective_saved >= target:
        if goal.saved_amount != target:
            goal.saved_amount = target
            fields.append("saved_amount")
        if goal.current_month_auto_allocation != Decimal("0"):
            goal.current_month_auto_allocation = Decimal("0")
            fields.append("current_month_auto_allocation")
        if goal.is_active:
            goal.is_active = False
            fields.append("is_active")
    elif goal.saved_amount > target:
        goal.saved_amount = target
        fields.append("saved_amount")
...
def _calculate_goal_allocations(available_amount: float, goals: list) -> dict:
    ...
    active_goals = [g for g in goals if _goal_is_fundable(g)]
    ...
    for goal in sorted_goals:
        ...
        remaining_needed = _goal_remaining_amount(goal)
    ...
    if remaining_pool > 0.01:
        for goal in sorted_goals:
            if remaining_pool <= 0:
                break
            still_needed = _goal_remaining_amount(goal) - allocations.get(goal.id, 0)
            if still_needed > 0:
...
def _build_weekly_category_comparison(user, this_week_start, this_week_end, last_week_start, last_week_end):
    """Compare category spending between this week and last week."""
    cat_labels = dict(Transaction.CATEGORY_CHOICES)

    this_week_txns = _active_expense_qs(
        Transaction.objects.filter(user=user, date__gte=this_week_start, date__lte=this_week_end)
    )
    last_week_txns = _active_expense_qs(
        Transaction.objects.filter(user=user, date__gte=last_week_start, date__lte=last_week_end)
    )

    this_week_cats = {}
    for row in this_week_txns.values('category').annotate(s=Sum('amount')):
        this_week_cats[row['category']] = float(row['s'] or 0)

    last_week_cats = {}
    for row in last_week_txns.values('category').annotate(s=Sum('amount')):
        last_week_cats[row['category']] = float(row['s'] or 0)

    this_week_total = sum(this_week_cats.values())
    last_week_total = sum(last_week_cats.values())

    all_categories = set(this_week_cats.keys()) | set(last_week_cats.keys())
    category_changes = []
    for cat in all_categories:
        this_amt = this_week_cats.get(cat, 0)
        last_amt = last_week_cats.get(cat, 0)
        if this_amt == 0 and last_amt == 0:
            continue
        if last_amt > 0:
        ...
        category_changes.append({
            ...
            'this_week': round(this_amt, 2),
            'last_week': round(last_amt, 2),
            'change_pct': change_pct,
            'direction': 'up' if change_pct > 0 else ('down' if change_pct < 0 else 'flat'),
        })

    category_changes.sort(key=lambda c: abs(c['change_pct']), reverse=True)

    # Top spending items this week
    top_items = []
    for txn in this_week_txns.order_by('-amount')[:5]:
        top_items.append({
            'title': txn.title,
            'amount': float(txn.amount),
            'category': txn.category,
    ...
    return {
        ...
        'total_change_pct': round(
            ...
            if last_week_total > 0 else (100.0 if this_week_total > 0 else 0.0),
            1
        ),
...
def _generate_weekly_ai_insight(comparison_data, saved_amount, salary):
    """Generate an AI or local weekly spending insight."""
    this_total = comparison_data['this_week_total']
    last_total = comparison_data['last_week_total']
    change_pct = comparison_data['total_change_pct']
    cats = comparison_data['category_changes']

    cat_summary = ', '.join(
        f"{c['label']}: Rs.{c['this_week']:.0f}" +
        (f" ({'up' if c['change_pct'] > 0 else 'down'}{abs(c['change_pct']):.0f}% vs last week)" if c['last_week'] > 0 else ' (new this week)')
        for c in cats[:4]
    )

    prompt = (
        f"You are a friendly personal finance advisor. Write a 2-3 sentence weekly spending insight. "
        f"This week's spending: Rs.{this_total:.0f}. Last week: Rs.{last_total:.0f} ({'up' if change_pct > 0 else 'down'} {abs(change_pct):.0f}%). "
        f"Category breakdown: {cat_summary}. "
        f"Monthly savings so far: Rs.{saved_amount:.0f}. "
        f"Be specific, actionable, and encouraging. Mention the biggest category change. Keep it under 50 words."
    )

    api_key = getattr(settings, 'OPENAI_API_KEY', '').strip()
    if api_key:
        model = (getattr(settings, 'OPENAI_MOTIVATION_MODEL', 'gpt-4.1-mini') or 'gpt-4.1-mini').strip()
        payload = {
            'model': model,
            'input': prompt,
            'temperature': 0.8,
        ...
        try:
            ...
            ai_text = _extract_openai_text(resp_data)
            if ai_text:
                return ai_text[:300], True
        except Exception:
            pass

    return _local_weekly_fallback(comparison_data, saved_amount), False


def _local_weekly_fallback(comparison_data, saved_amount):
    """Generate a local weekly insight without OpenAI."""
    this_total = comparison_data['this_week_total']
    last_total = comparison_data['last_week_total']
    change_pct = comparison_data['total_change_pct']
    cats = comparison_data['category_changes']

    rng = random.Random(int(date.today().strftime('%Y%m%d')) + int(this_total))

    if this_total == 0 and last_total == 0:
        return rng.choice([
            'No expenses this week or last — your wallet is taking a well-deserved vacation.',
            'Zero spending two weeks in a row? Either you are a monk or you forgot to log expenses.',
        ])

    biggest = cats[0] if cats else None
    biggest_text = ''
    if biggest:
        if biggest['change_pct'] > 20:
            biggest_text = f" {biggest['label']} jumped {biggest['change_pct']:.0f}% — worth keeping an eye on."
        elif biggest['change_pct'] < -20:
            biggest_text = f" Great job cutting {biggest['label']} by {abs(biggest['change_pct']):.0f}%."
        else:
            biggest_text = f" {biggest['label']} stayed steady at Rs.{biggest['this_week']:.0f}."

    if change_pct > 30:
        templates = [
            f"Your spending jumped {change_pct:.0f}% this week. Time to check what crept in.{biggest_text}",
            f"Week-over-week spending up {change_pct:.0f}%. {biggest_text} A no-spend day could help balance things out.",
        ]
    elif change_pct > 5:
        templates = [
            f"Spending is up {change_pct:.0f}% vs last week. Not alarming, but worth a quick review.{biggest_text}",
            f"A slight {change_pct:.0f}% uptick this week. {biggest_text} Stay mindful.",
        ]
    elif change_pct > -5:
        templates = [
            f"Spending held steady this week at Rs.{this_total:.0f}.{biggest_text} Consistency is a superpower.",
            f"Flat week at Rs.{this_total:.0f}. {biggest_text} You are running a tight ship.",
        ]
    elif change_pct > -30:
        templates = [
            f"Nice — spending dropped {abs(change_pct):.0f}% this week. {biggest_text} Keep the momentum.",
            f"Down {abs(change_pct):.0f}% week-over-week. {biggest_text} Your wallet is breathing easier.",
        ]
    else:
        templates = [
            f"Spending plummeted {abs(change_pct):.0f}%! {biggest_text} Either massive discipline or a quiet week. Well done.",
            f"Incredible — you cut spending by {abs(change_pct):.0f}%. {biggest_text} This is the energy.",
        ]
...
@login_required(login_url='/login/')
@require_http_methods(['GET'])
def api_weekly_insights(request: HttpRequest) -> JsonResponse:
    """Return AI-powered weekly spending insights."""
    user = request.user
    profile, _ = UserProfile.objects.get_or_create(user=user)
    today = date.today()

    this_week_start, this_week_end = _get_week_bounds(today)
    last_week_start = this_week_start - timedelta(days=7)
    last_week_end = this_week_end - timedelta(days=7)

    comparison = _build_weekly_category_comparison(
        user, this_week_start, this_week_end, last_week_start, last_week_end
    )

    month_start = today.replace(day=1)
    salary = Decimal(str(profile.salary)) if profile.salary else Decimal('0')
    month_expense = _active_expense_qs(
        Transaction.objects.filter(user=user, date__gte=month_start, date__lte=today)
    ).aggregate(s=Sum('amount'))['s'] or Decimal('0')

    from .models import ExcessIncome
    current_month_str = month_start.strftime('%Y-%m')
    excess_obj = ExcessIncome.objects.filter(user=user, month=current_month_str).first()
    excess = Decimal(str(excess_obj.amount)) if excess_obj else Decimal('0')
    saved = float((salary + excess) - month_expense) if salary > 0 else 0.0

    insight_text, is_ai = _generate_weekly_ai_insight(comparison, saved, float(salary))
    ...
    return JsonResponse({
        ...
        'month_saved': round(saved, 2),
    })


# ── Month-over-Month Comparison ─────────────────────────────
def _build_comparison_data(user, profile, month_a_start, month_b_start):
    """Build side-by-side comparison data for two months."""
    cat_labels = dict(Transaction.CATEGORY_CHOICES)

    def _month_data(ms):
        me_start, me_end = _month_bounds(ms)
        txns = Transaction.objects.filter(user=user, date__gte=me_start, date__lte=me_end)
        income = txns.filter(txn_type='income').aggregate(s=Sum('amount'))['s'] or Decimal('0')
        expense = _active_expense_qs(txns).aggregate(s=Sum('amount'))['s'] or Decimal('0')

        cats = {}
        for row in _active_expense_qs(txns).values('category').annotate(s=Sum('amount')):
            amt = float(row['s'] or 0)
            if amt > 0:
                cats[row['category']] = {
        ...
        cursor = me_start
        while cursor <= me_end:
            w_end = min(cursor + timedelta(days=6), me_end)
            w_exp = _active_expense_qs(
                Transaction.objects.filter(user=user, date__gte=cursor, date__lte=w_end)
            ).aggregate(s=Sum('amount'))['s'] or Decimal('0')
            w_inc = txns.filter(txn_type='income', date__gte=cursor, date__lte=w_end).aggregate(s=Sum('amount'))['s'] or Decimal('0')
            weeks.append({
                'label': f'W{len(weeks)+1}',
                'range': f'{cursor.day}-{w_end.day}',
                'income': float(w_inc),
                'expense': float(w_exp),
            })
        ...
        sorted_cats = sorted(cats.values(), key=lambda c: c['amount'], reverse=True)
        for c in sorted_cats:
            c['share_pct'] = round((c['amount'] / total_expense * 100), 1) if total_expense > 0 else 0

        return {
            'label': ms.strftime('%B %Y'),
            'short_label': ms.strftime('%b %Y'),
            'month_param': ms.strftime('%Y-%m'),
    ...
    data_a = _month_data(month_a_start)
    data_b = _month_data(month_b_start)

    expense_delta = data_a['expense'] - data_b['expense']
    income_delta = data_a['income'] - data_b['income']
    saved_delta = data_a['saved'] - data_b['saved']
    expense_pct = round((expense_delta / data_b['expense'] * 100) if data_b['expense'] > 0 else (100 if data_a['expense'] > 0 else 0), 1)
    income_pct = round((income_delta / data_b['income'] * 100) if data_b['income'] > 0 else (100 if data_a['income'] > 0 else 0), 1)
    saved_pct = round((saved_delta / abs(data_b['saved']) * 100) if data_b['saved'] != 0 else (100 if data_a['saved'] != 0 else 0), 1)

    all_cats = set(c['label'] for c in data_a['categories']) | set(c['label'] for c in data_b['categories'])
    cat_map_a = {c['label']: c for c in data_a['categories']}
    cat_map_b = {c['label']: c for c in data_b['categories']}
    category_deltas = []
    for label in all_cats:
        a_amt = cat_map_a.get(label, {}).get('amount', 0)
        b_amt = cat_map_b.get(label, {}).get('amount', 0)
        delta = a_amt - b_amt
        if a_amt == 0 and b_amt == 0:
            continue
        cat_delta_pct = round((delta / b_amt * 100) if b_amt > 0 else (100 if a_amt > 0 else -100), 1)
        category_deltas.append({
            'label': label,
            'month_a': round(a_amt, 2),
            'month_b': round(b_amt, 2),
            'delta': round(delta, 2),
            'change_pct': cat_delta_pct,
            'direction': 'up' if delta > 0 else ('down' if delta < 0 else 'flat'),
        })
    category_deltas.sort(key=lambda c: abs(c['delta']), reverse=True)
    ...
    return {
        ...
        'saved_delta': round(saved_delta, 2),
        'saved_pct': saved_pct,
...
def compare(request: HttpRequest) -> HttpResponse:
    ...
    month_a_param = request.GET.get('a')
    month_b_param = request.GET.get('b')

    if month_b_param:
        month_b = _parse_month_param(month_b_param)
    else:
        month_b = this_month

    if month_a_param:
        month_a = _parse_month_param(month_a_param)
    else:
        month_a = _add_months(this_month, -1)

    profile, _ = UserProfile.objects.get_or_create(user=request.user)
    comparison = _build_comparison_data(request.user, profile, month_a, month_b)
    ...
    return render(request, 'login/compare.html', {
        **comparison,
        'month_options': month_options,
        'selected_a': month_a.strftime('%Y-%m'),
        'selected_b': month_b.strftime('%Y-%m'),
        'active_nav': 'compare',
        'profile': profile,
    })
...
def api_compare(request: HttpRequest) -> JsonResponse:
    """API for month comparison data (for AJAX month switching)."""
    today = date.today()
    this_month = today.replace(day=1)
    month_a_param = request.GET.get('a')
    month_b_param = request.GET.get('b')

    month_a = _parse_month_param(month_a_param) if month_a_param else _add_months(this_month, -1)
    month_b = _parse_month_param(month_b_param) if month_b_param else this_month
