...
# ── Login via OTP ───────────────────────────────────────
# TODO: enable when OTP login is ready
...
#                 except Exception:
#                     errors["general"] = "Could not send OTP. Please try again."
#                     return render(request, "login/login_otp.html", {"errors": errors, "form": request.POST})
...
#
#
# def _send_login_otp_email(user: User, code: str) -> None:
#     expiry_minutes = getattr(settings, "EMAIL_VERIFICATION_CODE_EXPIRY_MINUTES", 10)
#     name = user.first_name or user.username or "there"
#     text_body = (
#         f"Hi {name},\n\n"
#         f"Use this 6-digit OTP to log in to SpendWise: {code}\n\n"
#         f"This OTP expires in {expiry_minutes} minutes.\n\n"
#         "If you did not request this, you can ignore this email."
#     )
#     html_body = render_to_string("login/emails/otp_email.html", {
#         "email_subject": "Your SpendWise Login OTP",
#         "email_heading": "Login OTP",
#         "email_subheading": f"Hi {name}, use the OTP below to log in to SpendWise.",
#         "otp_code": code,
#         "expiry_minutes": expiry_minutes,
#     })
#     send_mail(
#         subject="Your SpendWise Login OTP",
#         message=text_body,
#         from_email=settings.DEFAULT_FROM_EMAIL,
...
#
#
# def login_otp_verify(request: HttpRequest) -> HttpResponse:
#     """Step 2: user enters OTP, gets logged in."""
#     if request.user.is_authenticated:
#         return redirect("dashboard")
#
...
#                 messages.success(request, f"A fresh OTP was sent to {user.email}.")
#             except Exception:
#                 errors["general"] = "Could not resend OTP. Please try again."
#             return render(request, "login/login_otp_verify.html", {"errors": errors, "email": user.email})
#
#         code = request.POST.get("otp", "").strip()
#         expiry_minutes = getattr(settings, "EMAIL_VERIFICATION_CODE_EXPIRY_MINUTES", 10)
#         sent_at = profile.email_verification_sent_at if profile else None
#
#         if not code:
#             errors["otp"] = "Enter the 6-digit OTP."
#         elif code != (profile.email_verification_code if profile else None):
#             errors["otp"] = "That OTP is incorrect."
#         elif not sent_at or timezone.now() > sent_at + timedelta(minutes=expiry_minutes):
#             errors["otp"] = "That OTP expired. Please resend a new one."
#
